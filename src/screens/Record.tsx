...
export default function Record({ back }: Props) {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      if (!online && !photo) return // 念のため（写真なしで日時だけ操作するケース）

      setTideLoading(true)
      try {
        const { series, source, isStale, tideName } = await getTide736DayCached(FIXED_PORT.pc, FIXED_PORT.hc, capturedAt, { ttlDays: 30 })
        if (cancelled) return

        const info = getTideAtTime(series as TidePoint[], capturedAt.getTime())
        const ph = getTidePhaseFromSeries(series as TidePoint[], capturedAt, capturedAt)
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, minWidth: 0 }}>
        ...
        {/* プレビュー */}
        {previewUrl && (
          <div className="glass glass-strong" style={{ borderRadius: 16, padding: 10, maxWidth: 740 }}>
        ...
        {/* 潮プレビュー */}
        {photo && (
          <div className="glass glass-strong" style={{ borderRadius: 16, padding: 12, maxWidth: 740 }}>
            <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
              <div style={{ fontWeight: 800 }}>🌙 タイド（プレビュー）</div>
              {!online && <div style={{ fontSize: 12, color: '#f6c' }}>📴 オフライン</div>}
              {tideSource &&
                (() => {
                  const lab = sourceLabel(tideSource, tideIsStale)
                  if (!lab) return null
                  return (
                    <div style={{ fontSize: 12, color: lab.color, whiteSpace: 'nowrap' }} title="tide736取得元">
                      🌊 {lab.text}
                    </div>
                  )
                })()}
            ...
            {!capturedAt ? (
              <div style={{ marginTop: 8, fontSize: 12, color: 'rgba(255,255,255,0.68)' }}>撮影日時が無いので、タイドに紐づけできないよ</div>
            ) : tideLoading ? (
              <div style={{ marginTop: 8, fontSize: 12, color: '#0a6' }}>取得中…</div>
            ) : tideError ? (
              <div style={{ marginTop: 8, fontSize: 12, color: '#ff7a7a' }}>取得失敗 → {tideError}</div>
            ) : (
              <div style={{ marginTop: 8, display: 'grid', gap: 6 }}>
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.75)' }}>🕒 {getTimeBand(capturedAt)}</div>
                <div style={{ fontSize: 12, color: '#6cf' }}>
                  {tideName ? `🌙 ${tideName}` : '🌙 潮名：—'}
                  {phase ? ` / 🌊 ${phase}` : ''}
                </div>
                <div style={{ fontSize: 12, color: '#7ef' }}>
                  🌊 焼津潮位：{tideAtShot ? `${tideAtShot.cm}cm / ${tideAtShot.trend}` : '—'}
                </div>
                {!online && tideSource === 'stale-cache' && (
                  <div style={{ marginTop: 4, fontSize: 12, color: '#f6c' }}>⚠ オフラインのため、期限切れキャッシュの可能性あり</div>
                )}
              </div>
            )}
          </div>
        ...
        <div>
          ...
          <div className="glass glass-strong" style={{ ...glassBoxStyle, maxWidth: 740 }}>
