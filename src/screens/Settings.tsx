...
export default function Settings({ back }: Props) {
  ...
  const fixedCharacterId = settings?.fixedCharacterId ?? characterOptions[0]?.id ?? DEFAULT_SETTINGS.fixedCharacterId

  const characterScale = Number.isFinite(settings?.characterScale) ? (settings.characterScale as number) : DEFAULT_SETTINGS.characterScale
  const characterOpacity = Number.isFinite(settings?.characterOpacity) ? (settings.characterOpacity as number) : DEFAULT_SETTINGS.characterOpacity

  const bgDim = Number.isFinite(settings?.bgDim) ? (settings.bgDim as number) : DEFAULT_SETTINGS.bgDim
  const bgBlur = Number.isFinite(settings?.bgBlur) ? (settings.bgBlur as number) : DEFAULT_SETTINGS.bgBlur

  const infoPanelAlpha = Number.isFinite(settings?.infoPanelAlpha) ? (settings.infoPanelAlpha as number) : DEFAULT_SETTINGS.infoPanelAlpha
  const infoPanelBlur = Number.isFinite(settings?.infoPanelBlur) ? (settings.infoPanelBlur as number) : DEFAULT_SETTINGS.infoPanelBlur
  ...
  return (
    <PageShell
      title={<h1 style={{ margin: 0, fontSize: 'clamp(20px, 5.5vw, 32px)' }}>⚙ 設定</h1>}
      ...
      <div style={{ display: 'grid', gap: 16 }}>
          ...
          <div style={formGrid}>
            ...
            <div style={row}>
              ...
              <div style={{ ...radioLine, opacity: characterEnabled ? 1 : 0.5 }}>
                <label style={{ display: 'inline-flex', gap: 8, alignItems: 'center', cursor: characterEnabled ? 'pointer' : 'not-allowed' }}>
                  <input type="radio" name="characterMode" checked={characterMode === 'fixed'} disabled={isCharControlsDisabled} onChange={() => set({ characterMode: 'fixed' })} />
                  <span>固定</span>
                </label>

                <label style={{ display: 'inline-flex', gap: 8, alignItems: 'center', cursor: characterEnabled ? 'pointer' : 'not-allowed' }}>
                  <input type="radio" name="characterMode" checked={characterMode === 'random'} disabled={isCharControlsDisabled} onChange={() => set({ characterMode: 'random' })} />
            ...
            <div style={row}>
              ...
              <div style={rowStack}>
                <select value={fixedCharacterId} disabled={isFixedDisabled} onChange={(e) => set({ fixedCharacterId: e.target.value })} style={fullWidthControl}>
            ...
            <div style={row}>
              ...
              <div style={rowStack}>
                ...
                <input
                  ...
                  onChange={(e) => set({ characterScale: clamp(Number(e.target.value), 0.7, 5.0) })}
          ...
          <div style={formGrid}>
            <div style={row}>
              ...
              <div style={rowStack}>
                ...
                <input type="range" min={0} max={1} step={0.02} value={bgDim} onChange={(e) => set({ bgDim: clamp(Number(e.target.value), 0, 1) })} style={fullWidthControl} />
              </div>
            </div>
            ...
            <div style={row}>
              ...
              <div style={rowStack}>
                ...
                <input type="range" min={0} max={24} step={1} value={bgBlur} onChange={(e) => set({ bgBlur: clamp(Number(e.target.value), 0, 24) })} style={fullWidthControl} />
              </div>
            </div>
            ...
            <div style={row}>
              ...
              <div style={rowStack}>
                ...
                <input type="range" min={0} max={0.85} step={0.05} value={infoPanelAlpha} onChange={(e) => set({ infoPanelAlpha: clamp(Number(e.target.value), 0, 1) })} style={fullWidthControl} />
              </div>
            </div>
            ...
            <div style={row}>
              ...
              <div style={rowStack}>
                <div style={controlLine}>
                  <span style={help}>情報板の blur（端末で効き方が変わる）</span>
                  <span style={help}>{infoPanelBlur}px</span>
                </div>
                <input type="range" min={0} max={24} step={1} value={infoPanelBlur} onChange={(e) => set({ infoPanelBlur: clamp(Number(e.target.value), 0, 24) })} style={fullWidthControl} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="glass glass-strong" style={card}>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
            <button type="button" style={loading || !!busy ? pillDisabled : pillBase} disabled={loading || !!busy} onClick={() => refresh()}>
              ↻ 更新
            </button>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button
            ...
            style={pillBase}
            onClick={() => {
              const ok = confirm('設定ストレージを削除して再読み込みする？（復旧用）')
              if (!ok) return
              try {
