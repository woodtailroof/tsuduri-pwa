...
export const PRESETS: CharacterProfile[] = [
  {
    ...
    formality: 20,
    systemNote:
      "元気で可愛い、少し甘え＆少し世話焼き。釣りは現実的に頼れる相棒。説教禁止、心配として言う。",
...
function normalizeProfile(
  ...
  return {
    ...
    label:
      typeof x?.label === "string" && x.label.trim() ? x.label : fallback.label,
    selfName:
      typeof x?.selfName === "string" && x.selfName.trim()
        ? x.selfName
        : fallback.selfName,
    callUser:
      typeof x?.callUser === "string" && x.callUser.trim()
        ? x.callUser
        : fallback.callUser,
    temperature: clamp(
      Number(x?.temperature ?? fallback.temperature),
      0.2,
      1.2
    ),
    ...
    systemNote:
      typeof x?.systemNote === "string" ? x.systemNote : fallback.systemNote,
    portraitSrc:
      typeof x?.portraitSrc === "string" ? x.portraitSrc : fallback.portraitSrc,
  };
}
...
export function loadCharacterState(): CharacterState {
  try {
    ...
    const arr = Array.isArray((j as any).characters)
      ? (j as any).characters
      : [];
    ...
    const activeId =
      typeof (j as any).activeId === "string"
        ? (j as any).activeId
        : normalized[0].id;
...
export function upsertCharacter(profile: CharacterProfile) {
  ...
  const activeId =
    st.activeId && next.some((c) => c.id === st.activeId)
      ? st.activeId
      : next[0].id;
  ...
  try {
    ...
    return parsed
      ...
      .map((m) => ({
        ...
        speakerLabel:
          typeof m.speakerLabel === "string" ? m.speakerLabel : undefined,
      }));
  } catch {
    return [];
  }
...
export function useCharacterStore() {
  ...
  const api = useMemo(
    () => ({
      ...
      setPortraitSrc: (id: string, src: string) =>
        setCharacterPortraitSrc(id, src),
    }),
    []
  );
