...
export const onRequestGet: PagesFunction = async (context) => {
  ...
  for (const key of ["pc", "hc", "yr", "mn", "dy", "rg"]) {
    const value = reqUrl.searchParams.get(key);
    if (value != null && value.trim()) upstreamParams.set(key, value.trim());
  }
  ...
  const upstreamOrigin =
    (context.env as any)?.TIDE736_ORIGIN?.toString()?.trim() ||
    "https://api.tide736.net";
  ...
  try {
    upstreamRes = await fetch(upstreamUrl, {
      ...
      headers: {
        ...
        "User-Agent":
          "Mozilla/5.0 (compatible; tsuduri-pwa/1.0; +https://pages.dev)",
      },
    });
  } catch {
    return json(
      {
        status: false,
        error: "tide736 upstream fetch failed",
      },
      502,
    );
  }
